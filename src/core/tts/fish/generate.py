...
if hasattr(torch._inductor.config, "fx_graph_cache"):
    ...
    # NOTE: seems needs to be False in chainlit
...
class LlamaGenerator:
    def __init__(
        ...
        if prompt_tokens is not None:
            prompt_tokens = [
                torch.from_numpy(np.load(p)).to(device) for p in prompt_tokens
            ]
    ...
    def run(self, text: str):
        ...
        for response in self.generator(text=text):
